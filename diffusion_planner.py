...
def replace_submodules(root_module, predicate, func):
    ...
    for *parent, k in bn_list:
        parent_module = root_module.get_submodule('.'.join(parent)) if len(parent) > 0 else root_module
...
def replace_bn_with_gn(root_module, features_per_group: int = 16):
    replace_submodules(
...
class DiffusionPlanner:
    """
    Diffusion-based 2D path planner. Takes occupancy map image + start/goal, outputs trajectory.
    """
    def __init__(self, device=None):
        ...
        self.obs_dim = vision_feature_dim + 2 + 2
        self.action_dim = 2
        self.obs_horizon = 1
        ...
        self.nets = self.nets.to(torch.device(device))
    ...
    def plan(self, map_img: np.ndarray, start: np.ndarray, goal: np.ndarray,
             diffusion_iters: int = 1000, path_steps: int = 64) -> np.ndarray:
        """
        map_img: (H, W, 3) float32 in [0,1], white=free
        start, goal: (2,) or (1,2) in meters, range [-5, 5]
        """
        start = np.asarray(start, dtype=np.float32).reshape(1, 2)
        goal = np.asarray(goal, dtype=np.float32).reshape(1, 2)

        # Be permissive: accept uint8 maps and normalize to [0,1]
        if isinstance(map_img, np.ndarray) and map_img.max() > 1:
            map_img = (map_img.astype(np.float32) / 255.0)

        images = np.stack([np.moveaxis(map_img, -1, 0) for _ in range(self.obs_horizon)])
        nstart = normalize_data(start, self.stats)
        ngoal = normalize_data(goal, self.stats)

        nimages = torch.from_numpy(images).to(self.device, dtype=torch.float32)
        nstart = torch.from_numpy(nstart).to(self.device, dtype=torch.float32)
        ...
        with torch.no_grad():
            ...
            noisy_action = torch.randn((1, path_steps, self.action_dim), device=self.device)
            noisy_action[0, 0] = nstart
            noisy_action[0, -1] = ngoal
            naction = noisy_action

            diffusion_iters = int(min(diffusion_iters, self.noise_scheduler.config.num_train_timesteps))
            self.noise_scheduler.set_timesteps(diffusion_iters)
            for k in self.noise_scheduler.timesteps:
                noise_pred = self.nets['noise_pred_net'](naction, k, global_cond=obs_cond)
                naction = self.noise_scheduler.step(noise_pred, k, naction).prev_sample
                naction[0, 0] = nstart
                naction[0, -1] = ngoal

            action_pred = unnormalize_data(naction[0].cpu().numpy(), self.stats)
        return action_pred
